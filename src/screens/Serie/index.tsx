...
import { SerieDetailsDTO } from 'dtos/SerieDetailsDTO';
...
export function Serie() {
  const [details, setDetails] = useState<SerieDetailsDTO>(
    {} as SerieDetailsDTO,
  );
  ...
  const { params } = useRoute<RootRouteProps<'Serie'>>();
  ...
  useEffect(() => {
    async function getData() {
      ...
      const detailsPromise = tmdb.get<SerieDetailsDTO>(
        `/tv/${params.id}?api_key=0b8d03de32e4889351d128d59380611b&language=pt-BR`,
      );

      const castPromise = tmdb.get<CastDTO>(
        `/tv/${params.id}/credits?api_key=0b8d03de32e4889351d128d59380611b`,
      );

      const galleryPromise = tmdb.get<GalleryResponse>(
        `/tv/${params.id}/images?api_key=0b8d03de32e4889351d128d59380611b`,
      );
        ...
        <>
          ...
          >
            <S.Info>
              ...
              <S.Details>
                {new Date(params.first_air_date).getFullYear()} |{' '}
                {details?.genres
                  .map(item => item.name)
                  .slice(0, 3)
                  .join(', ')}{' '}
              ...
              <S.SlideWrapper>
                <S.TitleSecondary>Temporadas</S.TitleSecondary>
